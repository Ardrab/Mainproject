...
from .models import Booking, CollectionStatus, TestResult, TestValue, LabTechnician, TestType

def download_test_report_by_booking(request, booking_id):
    booking = get_object_or_404(Booking, pk=booking_id)
    collection_status = get_object_or_404(CollectionStatus, booking=booking)
    
    # Fetch the lab technician
    lab_technician = collection_status.technician

    # Static values for testing date, collection date, and doctor's name
    ...
    # Header Section with Logo (replace 'path/to/logo.png' with the actual path)
    elements.append(Paragraph("MedLab Diagnostic Center", title_style))
    elements.append(Paragraph(f"123 Health Street, City, Country", subtitle_style))
    ...
    # Conducted By and Report Completed By
    ...
    table_data = [["Test Name", "Result", "Normal Range"]]
    test_results = TestResult.objects.filter(collection_status=collection_status).prefetch_related('values', 'test_types')
    ...
    for result in test_results:
        ...
        for test_type, value in zip(test_types, result_values):
            ...
            for single_value in split_values:
                row = [test_type.tests_names, single_value.strip(), normal_range]
    ...
    elements.append(Spacer(1, 0.3 * inch))
    elements.append(PageBreak())  # Add a page break for a clean layout if needed
